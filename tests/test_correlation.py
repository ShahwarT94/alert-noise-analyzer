...
def test_three_alert_cascading_cluster():
    ...
    alerts = [
        ...
        make_alert("V-002", "Azure Gateway Latency", "ACC-001", base_time + timedelta(minutes=2)),
        make_alert("V-003", "AWX Job Failure", "ACC-001", base_time + timedelta(minutes=4)),
    ]

    findings = detect_correlated_clusters(alerts, cluster_window_minutes=5, min_cluster_size=2)

    assert len(findings) == 1
    assert findings[0]["account_id"] == "ACC-001"
    assert findings[0]["alert_count"] == 3
    assert set(findings[0]["conditions_involved"]) == {"Host Down", "Azure Gateway Latency", "AWX Job Failure"}
...
def test_different_accounts_not_clustered():
    ...
    alerts = [
        ...
        make_alert("V-003", "AWX Job Failure", "ACC-001", base_time + timedelta(minutes=2)),
    ]
...
def test_alerts_outside_window_not_clustered():
    ...
    alerts = [
        make_alert("V-001", "Host Down", "ACC-001", base_time),
        make_alert("V-002", "Azure Gateway Latency", "ACC-001", base_time + timedelta(minutes=10)),
        make_alert("V-003", "AWX Job Failure", "ACC-001", base_time + timedelta(minutes=20)),
    ]
...
def test_duplicate_violation_id_in_cluster():
    ...
    alerts = [
        ...
        make_alert("V-002", "Azure Gateway Latency", "ACC-001", base_time + timedelta(minutes=2)),
        make_alert("V-003", "AWX Job Failure", "ACC-001", base_time + timedelta(minutes=3)),
    ]
...
def test_zero_alerts_returns_empty():
    ...
    findings = detect_correlated_clusters([], cluster_window_minutes=5, min_cluster_size=2)
    assert findings == []
...
def test_no_clusters_found_returns_empty():
    ...
    findings = detect_correlated_clusters(alerts, cluster_window_minutes=5, min_cluster_size=3)
...
def test_missing_account_id_skipped():
    ...
    alerts = [
        make_alert("V-001", "Host Down", "ACC-001", base_time),
        {"violation_id": "V-002", "condition": "Azure Gateway Latency", "opened_at_utc": (base_time + timedelta(minutes=1)).strftime("%Y-%m-%dT%H:%M:%SZ")},
        make_alert("V-003", "AWX Job Failure", "ACC-001", base_time + timedelta(minutes=2)),
    ]
...
def test_missing_opened_at_utc_skipped():
    ...
    alerts = [
        make_alert("V-001", "Host Down", "ACC-001", base_time),
        {"violation_id": "V-002", "condition": "Azure Gateway Latency", "account_id": "ACC-001"},
        make_alert("V-003", "AWX Job Failure", "ACC-001", base_time + timedelta(minutes=2)),
    ]
...
def test_bad_timestamp_format_skipped():
    ...
    alerts = [
        make_alert("V-001", "Host Down", "ACC-001", base_time),
        {"violation_id": "V-002", "condition": "Azure Gateway Latency", "account_id": "ACC-001", "opened_at_utc": "not-a-date"},
        make_alert("V-003", "AWX Job Failure", "ACC-001", base_time + timedelta(minutes=2)),
    ]
...
def test_findings_sorted_by_alert_count_desc_then_start_asc():
    ...
    alerts = []
    for i in range(3):
        alerts.append(make_alert(f"V-{i:03d}", f"Alert {i}", "ACC-001", base_time + timedelta(minutes=i * 2)))
    for i in range(5):
        alerts.append(make_alert(f"V-{i+3:03d}", f"Alert {i+3}", "ACC-002", base_time + timedelta(minutes=i * 2)))
    for i in range(4):
        alerts.append(make_alert(f"V-{i+8:03d}", f"Alert {i+8}", "ACC-003", base_time + timedelta(hours=1, minutes=i * 2)))
